...
export const getFromCache = async <T>(
  ...
  try {
    ...
    console.error(
      `   ${colors.yellow}⚠ CACHE ERROR${colors.reset} → Falló lectura:`,
      error,
    );
    return null;
  }
...
export const setInCache = async <T>(
  ...
  try {
    ...
    console.error(
      `   ${colors.yellow}⚠ CACHE ERROR${colors.reset} → Falló escritura:`,
      error,
    );
  }
};
...
export const invalidateCache = async (): Promise<void> => {
  try {
    ...
    if (keys.length > 0) {
      ...
      console.log(
        `   ${colors.green}✔ CACHE CLEARED${colors.reset} → ${keys.length} keys eliminadas`,
      );
    }
  } catch (error) {
    console.error(
      `   ${colors.yellow}⚠ CACHE ERROR${colors.reset} → Falló invalidación:`,
      error,
    );
  }
};
